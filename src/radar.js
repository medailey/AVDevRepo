       ...
       function createRadar() {
           ...
           if (showChartOnPage) {
               $.getJSON(dataLocation + "region.json", function (data) {
                   ...
                   if (chartData === undefined && $('#' + id + '_id div.lbltooltip').length == 0) {
            ...
            if(! $.fn.DataTable.isDataTable('#'+id+'-datatable-table')) {
                ...
                $('#' + id + '-datatable-table').DataTable({
                    ...
                    columns: columnsDT
                           ...
                           }).map(csv.filter(function (e) {
                               return $.inArray(e[1], independentscale) == -1;
                           }));

                           var findmin = Object.keys(axesInfo).map(function (key) {
                               return axesInfo[key].min;
                           });
                           var overallmin = Math.min.apply(null, findmin);
                           var findmax = Object.keys(axesInfo).map(function (key) {
                               return axesInfo[key].max;
                           });
                           ...
                           for (var c = 0; c < rolledUpMaps.length; c++) {
                               ...
                               Object.keys(currentrollupMap).forEach(function (chartName) {
                                   axesData = [];
                                   var existingChartId;
                                   if (chartData.map(function (e) {
                                       return e.chartName;
                                   }).indexOf(chartName) > -1) {
                                       var indx = chartData.map(function (e) {
                                           return e.chartName;
                                       }).indexOf(chartName);
                                   ...
                                   if ($.inArray(chartName, independentscale) > -1) {
                                       var indyChart = independentAxesCharts.filter(function (ch) {
                                           return ch.name == chartName;
                                       });
                       ...
                       var labelTooltip = d3.select('#' + id + '-div').append("div").attr("class", "lbltooltip").style("opacity", 0);
                   }
           ...
           function createCharts() {
               var radarChartContainer = d3.select("#" + id + "-chart-container");
               ...
               var columns = radarChartContainer.selectAll("#" + id + "-div .radar-column");
               columns[0].forEach(function (d, columnIndex) {
                   ...
                   function getChartId(d) {
                       var chartid = id + "-radar-" + d.chartId;
                       return chartid;
                   }
                   ...
                   chartSvgs.each(function (d) {
                       RadarChart('#' + getChartId(d), chartData.filter(function (chartDatum) {
                           return chartDatum.chartId == d.chartId;
                       }), radarChartOptions);
                   }); //end each svg
               }) //end each column
               $(function () {
