...
function sunburst(id,indx) {
	...
	function createSunburst() {
        ...
        function createVisualization() {
            ...
            var sunburstBounds = d3.select("#"+id+"-chart").node().getBoundingClientRect();
            ...
            svg = d3.select("#"+id+"-chart").append("svg").attr("id", id+"-container")
                .attr("width", width)
                .attr("height", height)
                .append("g")
        ...
        function showNodeExplanation(node) {
            ...
            d3.select("#"+id+"-current-node-value").text(getReadableValueString(node.value));
            d3.select("#"+id+"-explanation").style("visibility", "").classed("sunburst-negative", function (d) {
...
// 			});
        ...
        function drawLegend() {
            ...
            var legend = d3.select("#"+id+"-legend").append("svg:svg").attr("width", legendBoxWidth + ((maxDepth - 1) * legendDepthIndent)).attr("height", totalLegendHeight).on("mouseleave", function () {
// 				unDimClass(nodeVisuals);
// 				unDimClass(legendRects);
                d3.select("#"+id+"-explanation").style("visibility", "hidden");
            });
