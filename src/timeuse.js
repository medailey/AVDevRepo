...
function timeuse (id,indx) {
 ...
 $.getJSON(dataLocation + "region.json", function (data) {
     ...
     if (data["scenarios"][scenario].visualizations != undefined) {
         if (data["scenarios"][scenario].visualizations["TimeUse"][indx].file) {
     ...
     url += "/" + fileName;
            ...
            nv.addGraph({
                generate: function () {
                    var chart = nv.models.stackedAreaChart().margin({
                        ...
                        .clipEdge(true).id(id+"-stackedAreaChart").useInteractiveGuideline(true) //Tooltips which show all data points. Very nice!
        ...
        function drawLegend(personTypes) {
            ...
            var legend = d3.select("#"+id+"-legend").append("svg:svg").attr("width", legendBoxWidth).attr("height", totalLegendHeight);
